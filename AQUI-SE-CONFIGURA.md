...
1.  **Validación de `X-Api-Key`**: Se implementó un middleware de seguridad en `backend/src/middlewares/auth.middleware.ts` que protege la API. Todas las rutas conversacionales exigen esta clave.
2.  **Endpoint de FAQ (`/chat/faq/:category`)**: Creamos el servicio [faq.service.ts](file:///c:/Users/carlo/OneDrive/Desktop/chat%20bot/backend/src/services/faq.service.ts) que responde preguntas frecuentes sobre envíos, pagos, productos y devoluciones (las cuatro categorías del contrato).
...
3.  **Base de Datos en Supabase (Persistencia)**:
    Si el docente exige persistencia final, faltaría conectar Supabase en `backend/src/services/sessionStore.service.ts` para registrar el historial de conversaciones de forma permanente en la base de datos PostgreSQL, en lugar de en la memoria RAM local.
